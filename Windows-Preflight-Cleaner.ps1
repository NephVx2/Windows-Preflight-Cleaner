<#
.SYNOPSIS
    Windows-Preflight-Cleaner v5.3.10 - Windows 11 maintenance and cleanup suite
...
trap {
    try {
        $crashLog = Join-Path $env:TEMP "Windows-Preflight-Cleaner-crash.log"
        "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $($_.Exception.GetType().FullName): $($_.Exception.Message)`r`n$($_.ScriptStackTrace)`r`n---" |
            Out-File -LiteralPath $crashLog -Append -Encoding utf8
    } catch {}
    Write-Host ""
    Write-Host "====================================================" -ForegroundColor Red
    Write-Host "  FATAL ERROR: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "  (also logged to $env:TEMP\Windows-Preflight-Cleaner-crash.log)" -ForegroundColor Red
    Write-Host "====================================================" -ForegroundColor Red
...
if (-not $CurrentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    # Resolve the exact path of the executable currently running this script
    # (rather than the bare name 'pwsh') so the elevated relaunch works
    # regardless of where/how pwsh.exe is installed (Program Files, Microsoft
    # Store, portable...). A bare name relies on -Verb RunAs's ShellExecute
    # PATH lookup, which can silently fail to elevate on some installs.
    ...
    # Start-Process -ArgumentList joins an array into a single command-line
    # string WITHOUT quoting elements that contain spaces. $PSCommandPath
    # (or a -SkipTargets/-OnlyTargets value) can contain spaces - e.g. a
    # folder literally named "Script Anglais" - which then silently split
    # into extra arguments and made the elevated relaunch exit immediately,
    # before ever reaching the script body. This was the actual root cause
    # of the double-click launch silently failing while a direct "Run as
    # administrator" (which never goes through this code path) worked fine.
    # Build one pre-quoted command-line string instead of handing an array
    # to -ArgumentList.
    $RelayCommandLine = ($RelayArgs | ForEach-Object {
        $value = [string]$_
    ...
    # A relaunch through Windows Terminal (wt.exe) was tried here to match
    # Explorer's nicer console rendering, but Windows Terminal's default
    # profile doesn't auto-close on a non-zero exit code (e.g. when locked
    # targets make the script exit 1), leaving a second "press Enter to
    # restart" prompt stacked on top of the script's own - confusing, not
    # an improvement. Reverted to the plain relaunch below, which closes
    # cleanly on every exit code.
    try {
        Start-Process -FilePath $HostExePath -Verb RunAs -ArgumentList $RelayCommandLine -ErrorAction Stop
    } catch {
        # UAC denied/cancelled, or the relaunch genuinely failed to start:
        # surface it instead of exiting silently, so a double-click doesn't
        # just look like "nothing happened".
        Write-Host ""
        Write-Host "Elevation was cancelled or failed to start: $($_.Exception.Message)" -ForegroundColor Red
        if (-not $Silent) { Read-Host "Press ENTER to close" | Out-Null }
        exit 2
    }
    exit
...
function Format-Size {
    ...
    if ($Bytes -ge 1GB) { return ($Bytes / 1GB).ToString("N2", $ci) + " GB" }
    elseif ($Bytes -ge 1MB) { return ($Bytes / 1MB).ToString("N2", $ci) + " MB" }
    elseif ($Bytes -ge 1KB) { return ($Bytes / 1KB).ToString("N2", $ci) + " KB" }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDZHBDTDiVIwoyf
# Kf0mdz1ZLcDqpU9RACHtG+796TLK0aCCAygwggMkMIICDKADAgECAhB6X4r8AlBU
...
# ARUwLwYJKoZIhvcNAQkEMSIEINniNp5Kr/vNvNAtsTj0z7jbGI5EPHn+53R7XUjQ
# cowaMA0GCSqGSIb3DQEBAQUABIIBAGipSNRnjDhhERuQAMLZqR6bu4bAzzeOH3Hm
# /TDoTvws6LSaZ7t2Do0HWjlUJjNlQmxNBegmhoFrJKMytZ4e/v3/po2eYD2Cgsco
# 1Vx04UE3+bOPg7qfhXVMoJKdb+d/gpFFHZAhuo82RII9jz4IqDITE3llL4MmVvKf
# ZePRBiKnfWy3LB+NwxpiSZSjNwEZoeye1Vi6sjj3uzsEeRJF6RvB6wbqHjtq7AO4
# v13lNMsaYI3DWQfnCQCbjNDGzr7oca99hMlbMmUdGXSMZtfHKRJ8cNQXykCPz37n
# JHApUbqF8hnkx5AL4AXtUJFLwF3GPtFhtIr60tSPUgYxJOPJqHM=
